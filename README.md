...
TS не использует левосторонний стиль присваивания, как `int x = 0`. 
Аннотации типов всегда будут идти после присваивания.\
...
Типизирование каждого свойства необязательно. Если не определить тип, он примет тип `any`.
...
function printName(obj: { fist: string; last?: string }) {
  console.log(`${obj.fist.toUpperCase()}} ${obj.last?.toUpperCase()}`);
}
